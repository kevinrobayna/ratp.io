...
module RaptApiClient
  class ProfileSessionModel < ApiModelBase
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RaptApiClient::ProfileSessionModel` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RaptApiClient::ProfileSessionModel`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, deleted, created_on, created_by, modified_on, modified_by, name, description, profile_id, profile, brew_zilla_id, fermentation_chamber_id, hydrometer_id, still_id, temperature_controller_id, start_date, end_date, original_gravity, final_gravity, yeast_id, yeast, sent_alerts].hash
    end
