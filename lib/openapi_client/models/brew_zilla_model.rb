  ...
  class BrewZillaModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::BrewZillaModel` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::BrewZillaModel`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, deleted, created_on, created_by, modified_on, modified_by, name, serial_number, mac_address, device_type, active, disabled, username, connection_state, status, error, last_activity_time, rssi, firmware_version, is_latest_firmware, active_profile_id, active_profile_step_id, active_profile_session, customer_use, telemetry_frequency, compressor_delay, mode_switch_delay, cooling_hysteresis, profile_sessions, beta_updates, bluetooth_enabled, graph_zoom_level, temperature, target_temperature, min_target_temperature, max_target_temperature, total_run_time, cooling_enabled, cooling_run_time, cooling_starts, heating_enabled, heating_run_time, heating_starts, high_temp_alarm, low_temp_alarm, ntc_beta, ntc_ref_resistance, ntc_ref_temperature, pid_cycle_time, pid_enabled, pid_proportional, pid_integral, pid_derivative, sensor_differential, sensor_timeout, show_graph, sounds_enabled, temp_unit, use_internal_sensor, control_device_type, control_device_mac_address, control_device_temperature, heating_hysteresis, telemetry, auxillary_run_time, auxillary_starts, heating_utilisation, pump_enabled, pump_utilisation].hash
    end
